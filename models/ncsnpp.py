...
import pdb
from models import layerspp, layers
...
from models.layers import get_act
...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.act = act = get_act(config.model.act)
        ...
        # These are the channels that will be modeled
        categorical_channels = config.data.categorical_channels

        # Downsampling block

        in_channels = config.data.categorical_channels + config.data.continuous_channels
            ...
            if progressive != "none":
                if i_level == num_resolutions - 1:
                    ...
                    if progressive == "output_skip":
                        ...
                        modules.append(
                            conv3x3(in_ch, in_channels, bias=True, init_scale=init_scale)
                        )
                        pyramid_ch = in_channels
                    elif progressive == "residual":
    ...
    def forward(self, x, time_cond):
        ...
        # Will be in logit range [log(1e-5), log(1)]
